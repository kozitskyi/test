...
class User
{
    ...
    //активируем пользователя, если он подтвердил свой email
    public function activationEmail($act_code)
    {
        ...
        while (($row = $all_users->fetch_assoc()) != false) {
            ...
            {
                $this->connect()->query("UPDATE `users` SET status='2' WHERE activation='$act_code'");
                return true;
            }
        }
        $this->err = "Вы уже прошли подтверждение или ваша ссылка на подтверждение не валидна";
        return false;
    }
    ...
    public function sendMail($act){
        ...
        $message = "Здравствуйте! Для подтверждения регистрации перейдите пожалуйста поссылке: http://$host/activate.php?activation=".$act;
...
('$this->name','$this->login', '$this->password','$this->email','$act',1)");
            return true;
        }
        else return false;
    }
